  ...
  <div class="root">
      <div class="finish">
        <div class="finish__item">
          <div class="title">Lotto Results</div>
        </div>
        <div class="finish__item">
          <div class="btn-group">
            <button :class="tab == 'your history' ? 'btn btn--active':'btn'" @click="tab = 'your history'">Your Ticket History</button>
            <button :class="tab == 'all history' ? 'btn btn--active':'btn'" @click="tab = 'all history'" :disabled="resultAll==null">All History</button>
          </div>
        </div>
        <div class="finish__item">
          <div class="card" v-if="tab == 'your history'">
            ...
            <div class="card__item" style="padding: 70px;" v-if="!isMetaMaskConnected">
              <div style="display: flex; flex-direction: column; align-items: center; width: 100%;">
                <div style="padding: 10px">Connect wallet to check your tickets</div>
                <div style="padding: 10px">
                  <button class="button" @click="connectWallet">Unlock Wallet</button>
                </div>
              </div>
            </div>
            <template v-if="isMetaMaskConnected && result && result.status==3">
              ...
              <div v-show="cardExpand" class="card__item" style="border-top: 1px solid #ddd; align-items: flex-start;">
                  <div style="font-size: 1.2rem; font-weight: bold; width: 200px;">
                    <div style="">Winning Pool</div>
                    <div style="font-size: 2rem; color: var(--q-color-primary);">~${{winningPool}}</div>
                    <div style="font-size: .8rem;">{{winningPool}} BQB</div>
                  </div>
                  <div class="match-items">
                    <div class="match-items__item">
                      <div class="title">Match all 6</div>
                      <div class="value">{{bracketBQB[5]}} BQB</div>
                      <div class="sub-value">~${{bracketBQB[5]}}</div>
                    </div>
                    <div class="match-items__item">
                      <div class="title">Match any 5</div>
                      <div class="value">{{bracketBQB[4]}} BQB</div>
                      <div class="sub-value">~${{bracketBQB[4]}}</div>
                    </div>
                    <div class="match-items__item">
                      <div class="title">Match any 4</div>
                      <div class="value">{{bracketBQB[3]}} BQB</div>
                      <div class="sub-value">~${{bracketBQB[3]}}</div>
                    </div>
                    <div class="match-items__item">
                      <div class="title">Match any 3</div>
                      <div class="value">{{bracketBQB[2]}} BQB</div>
                      <div class="sub-value">~${{bracketBQB[2]}}</div>
                    </div>
                  </div>
              </div>
              <div v-show="cardExpand" class="card__item">
                <div>
                  <div style="font-size: 1rem;">Your rewards: 
                    <span style="font-size: 2rem; color: var(--q-color-primary);"> {{winningMine}}</span> BQB
                  </div>
                </div>                
              </div>
            </template>
            <div class="card__item" v-if="isMetaMaskConnected" style="justify-content: center;">
              <div v-if="cardExpand && result" class="card__expand__switcher" @click="cardExpand = !cardExpand">Hide <q-icon name="expand_less" color="info"/></div>
              <div v-if="!cardExpand && result" class="card__expand__switcher" @click="cardExpand = !cardExpand">Show <q-icon name="expand_more" color="info"/></div>
            </div>
          </div>
            ...
            <template v-if="result.status==3">
              ...
              <div v-show="cardExpand" class="card__item">
                <div style="width: 200px;">
                  <div style="font-size: 1rem;">Total Players Today: <span> {{result.amountOfPurchasedPeople}}</span></div>
                </div>                
              </div>
            </template>
...
export default {
  ...
  computed: {
    ...
    bracketBQB() {
      ...
      if(this.result!=null) {
        for(let i = 2;i<6;i++) {
          bracket[i] = BigNumber(this.result.amountCollectedInBQB)
    ...
    winningPool() {
      if(this.result!=null) 
        return BigNumber(this.result.amountCollectedInBQB).shiftedBy(-18).toFormat(2)
      return '-'
    },
    winningMine() {
      console.log(this.result.amountCollectedInBQB)
    ...
    winningNumber() {
      ...
      if(result!=null) {
        number[0] = BigNumber(result.finalNumber).shiftedBy(-10).mod(100).integerValue().toString()
        number[1] = BigNumber(result.finalNumber).shiftedBy(-8).mod(100).integerValue().toString()
        number[2] = BigNumber(result.finalNumber).shiftedBy(-6).mod(100).integerValue().toString()
        number[3] = BigNumber(result.finalNumber).shiftedBy(-4).mod(100).integerValue().toString()
        number[4] = BigNumber(result.finalNumber).shiftedBy(-2).mod(100).integerValue().toString()
        number[5] = BigNumber(result.finalNumber).mod(100).integerValue().toString()
      }
      return number
    },
    resultDate() {
      let date = this.result.endTime
