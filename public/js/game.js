...
var spinStartSpeed = 1; //spin starting speed
var spinEndSpeed = 1; //spin ending speed
var spinSpeed = 3; //spin speed
...
var gameData = {paused:true, sphereX:650, sphereY:325, cageRadius:225, radius:0, spin:false, selectNum:0, numberNum:0, 
	numberArray:[], selectArray:[], winArray:[], buttonArray:[], indexArray:[], dimArray:[], matchNum:0, ballsArray:[], cageArray:[], revealArray:[], totalBall:[], ballNumber:[]};
...
function startGame(){
	...
	itemNumberBg.visible = false;
	itemNumberSelectBg.visible = false;
	buttonLucky.visible = false;
...
function toggleNumber(obj){
	if(!obj.highlight.visible){
		if(gameData.selectNum < score_arr.length){
			...
			//console.log("Number ++ " + gameData.selectNum);
			//console.log("+ numberArray :: " + gameData.numberArray[gameData.selectNum]);
			//gameData.numberArray[n]
...
function randomizeNumber(){
	...
	resetCard();
	//shuffle(gameData.numberArray);
	//console.log("score_arr.length >> " + score_arr.length);

	person = prompt("Please enter your 6 winning number (saperated by comma)", "1,2,3,4,5,6");

	if(person === null){
		randomizeNumber();
	}else{
		my_nums = person.split(",");


		/*if (hasDuplicates(my_nums)) {
		  alert('Error: you have duplicates values !'+arr);
		}else{*/

			if(my_nums.length != 6){
				randomizeNumber();
			}else{
				for(var m = 0; m<my_nums.length; m++){

					my_nums[m] = parseInt(my_nums[m]-1);
					
				}

				for(var n=0; n<score_arr.length; n++){
					//toggleNumber(gameData.buttonArray[gameData.numberArray[n]].bg);	
					toggleNumber(gameData.buttonArray[my_nums[n]].bg);
...
function spinCage(){
	for(var n=0; n<gameData.cageArray.length; n++){
		...
		//console.log("spinCage: "+targetHit);
...
function beginWinNumberTimer(){
	TweenMax.to(ballsContainer, revealTimer, {overwrite:false, onComplete:revealWinNumber});	
}
...
function revealWinNumber(){
	...
	// var tweenNum = my_nums[gameData.numberNum];
	// var revealNum = my_nums[gameData.numberNum];
	//console.log("revealNum: "+revealNum+" = tweenNum:"+tweenNum);
	...
	currentNumber = parseInt(my_nums[gameData.numberNum]+1);
	...
	gameData.numberNum++;
	//gameData.ballsArray.pop(currentNumber);
	//removeItem(gameData.numberArray, currentNumber-1);
	//gameData.numberArray.pop((currentNumber+1));

	//arrayRemove(gameData.numberArray, currentNumber-1);
	gameData.numberArray.splice(gameData.numberArray.indexOf((currentNumber-1)),1);
